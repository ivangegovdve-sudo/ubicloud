...
class Prog::MachineImage::VersionMetalNexus < Prog::Base
  ...
  label def archive
    register_deadline("wait", source_vm_id ? source_vm.storage_size_gib * 24 : 3600)
  ...
  label def destroy
    ...
    miv = machine_image_version_metal.machine_image_version
    # Serialize with other `DestroyVersionMetal#destroy` labels of the same
  ...
  label def failed
    ...
    # the "wait" deadline here to avoid redundant pages.
    nap 30 * 60
  end
