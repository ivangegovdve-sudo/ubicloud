...
RSpec.describe Prog::MachineImage::VersionMetalNexus do
  ...
  describe "#archive" do
    before { refresh_frame(prog, new_values: {"source_vm_id" => source_vm.id}) }
  ...
  describe "#finish_archive" do
    ...
    it "skips source VM destroy when there is no source VM" do
      refresh_frame(prog, new_values: {"destroy_source_after" => true, "source_vm_id" => nil, "vm_host_id" => vm_host.id})
      expect(prog.sshable).to receive(:_cmd).with("sudo rm -f #{stats_path}")
      expect { prog.finish_archive }.to hop("wait")
    end
  ...
  describe "#destroy" do
    ...
    it "skips daemon teardown when status is not creating" do
      expect { prog.destroy }.to hop("wait_vms")
      expect(metal.reload.status).to eq("destroying")
    end
  ...
  describe "#failed" do
    it "naps 30 minutes" do
      expect { prog.failed }.to nap(30 * 60)
    end
  end

  describe "#source_vm" do
    it "returns nil when source_vm_id is absent" do
      expect(prog.source_vm).to be_nil
      expect(prog.source_vm).to be_nil
    end

    it "loads and caches the Vm when source_vm_id is set" do
      refresh_frame(prog, new_values: {"source_vm_id" => source_vm.id})
      expect(prog.source_vm.id).to eq(source_vm.id)
      expect(prog.source_vm.id).to eq(source_vm.id)
    end
  end

  describe "#vm_host" do
    it "derives from source_vm when source_vm_id is set" do
      refresh_frame(prog, new_values: {"source_vm_id" => source_vm.id})
      expect(prog.vm_host.id).to eq(vm_host.id)
    end

    it "falls back to VmHost[vm_host_id] when source_vm is absent" do
