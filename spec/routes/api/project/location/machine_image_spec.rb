...
RSpec.describe Clover, "machine-image" do
  ...
  describe "version destroy" do
    ...
    it "is idempotent when destroy is already scheduled" do
      2.times { delete "/project/#{project.ubid}/location/#{TEST_LOCATION}/machine-image/#{mi.name}/version/#{mi_version.version}" }
      expect(last_response.status).to eq(204)
      expect(mi_version_metal.destroy_set?).to be true
    end
  end
